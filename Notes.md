...
- If there is nothing on `medicine.txt` then the user will get "empty something liek that"
...
**in today's Note - 06-20-2026**

1. *we change the code of clearscreen from ANSI code to the stdlib using system() function because it works flaulessly across the system*
   - we use the preprocessor ```#ifdef , #else , #endif``` to get the type of operating system we are using the _WIN32 is automatically defined by the comiler when compiling the program.
2. *asd*
